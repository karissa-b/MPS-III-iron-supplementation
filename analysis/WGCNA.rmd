...
Genes which are co-expressed show similar changes across samples and are assuminlgy functionally related. This method does not rely on pre-defined gene sets. Rather, it clusters genes based on their
similarity into modules. These modules of genes can then be subjected to enrichment analysis to identify what they comprise of. They can represent pathways or functions that we already know about. However,
they can also represent new biological pathways or processes.

Here, I will perform WGNCA to identify what modules/clusters are associated with genotype, treatment and/or sex. WGCNA needs normalised data. I have already done this using the TMM method + CQN. The data was already filtered for lowly expressed genes as well. I will import these objects from the previous analyses in [RNA_seq](RNA_seq.html) doc.
...
Genes that do not vary across samples are not informative in network analysis, and will increase the noise. So I will only retain the top 75% most variable genes in the dataset 
...
After filtering, there will be `r dim(logCPM.filtered)[1]` genes retained from the original `r dim(logCPM)[1]` genes. 

The WGCNA pipeline is expecting an input matrix of RNAseq data (counts). Usually we need to rotate (transpose) the input data so `rows = samples` and `columns = gene counts`. So this is done here. An example of the first 5 rows is show here. 
...
Here we use the gene expression patterns after normalisation to build a sample dendrogram (sample tree), and check how the `traits` (e.g. metadata) related to the sample dendrogram.

Traits uses a design matrix as described in the previous DE genes analysis.
...
traits <-  
  model.matrix(~sex + genotype + treatment + genotype*treatment, x$samples) %>%
  as.data.frame
...
Also, I will need to choose between a signed or unsigned network. A good explanation of signed v unsigned networks is described [here](https://peterlangfelder.com/2019/05/30/signed-network-from-signed-topological-overlap/). However, essentially, in an *unsigned* network, genes are considered connected regardless of whether the correlation is positive or negative. Then in singed networks, the direction matters. 

I will prepare a signed AND an unsigned network and see what the results look like. First, generate the plots to pick the soft thresholds. 
...
From the 2 graphs, I will assign the soft powers for the signed and unsigned networks as  `r softpower.signed` and `r softpower.unsigned` respectively. This is the first value where the graphs plateau which is more than 0.8. 

## Co-expression adjacency

We now calculate the adjacency matrix, using the soft thresholding powers just described. This essentially takes the gene expression data and calculates the pairwise similarity between genes (Euclidean distance). This similarity measure is then transformed into an adjacency matrix by using the power function. (i.e. a = |s^β|)
...
actually I dont think my computer can do this when i am looking at all the genes. I'll just show a subset. Some genes show some correlation. (i.e. in the top left)
...
Plotting the dissimilairty of the genes in the network reveals a lot of the genes with TOM between 0.9 and 1. This means that the majority of the genes are very connected to each other.
...
The table below shows the number modules, and how many genes per module there are.
...
The next plot shows the clustering dendrogram of genes, with dissimilarity based on topological overlap, together with assigned merged module colors. There are a lot of modules here which are all over
the dendrogram. The plots i've seen previously (i.e. [here](https://www.google.com/search?q=wgcna+dendrogram&sca_esv=592686156&rlz=1C5CHFA_enAU758AU758&tbm=isch&sxsrf=AM9HkKkSBZcwsy5iPy7CeFt2oRp73UiS6A:1703121651127&source=lnms&sa=X&sqi=2&ved=2ahUKEwj675jIrp-DAxU6TGwGHfCTBwMQ_AUoAXoECAIQAw&biw=1239&bih=827&dpr=1#imgrc=FAA689FFNIDKNM)) often show at least some blocks of module colours on the dendrogram. Although this is not always the case (see
...
The Dynamic Tree Cut may identify modules whose expression profiles are very similar. It may be beneficial to merge such modules since their genes are highly co-expressed. To quantify co-expression similarity of entire modules, we calculate their eigengenes and cluster them on their
correlation.
...
There appear to be plenty of modules which show similarity. Here we define the threshold for module merging as a height < 0.25 (as performed by authors of WGCNA). This corresponds to correlation of 0.75,
...
I dont see a huge difference in the dendrogram. However, it did reduce the number of modules. Perhaps next I will do some enrichment analysis to see if the modules are biologically meaningful.
...
these have very specific and broad meaning. Since I am having trouble using the inbuilt functions in `WGCNA`, I will use `cluster{rofiler` which allows me to perform a Fishers exact test for over representation.  
