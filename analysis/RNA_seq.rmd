...
x %>% 
  cpm(log=T) %>% 
  .[GO$GO_MITOTIC_CELL_CYCLE_PROCESS ,] %>% 
  .[,male.fish.ids] %>% 
  pheatmap(
...
x %>% 
  cpm(log=T) %>% 
  .[GO$	GO_AMINO_SUGAR_CATABOLIC_PROCESS ,] %>% 
  .[,male.fish.ids] %>% 
  pheatmap(
...
```{r}
fry.kegg[c("hom_control_female - het_control_female", 
  "hom_control_male - het_control_male",
  "hom_iron_female - het_control_female",
  "hom_iron_male - het_control_male")] %>% 
  lapply(function(x) {
...
fry.ire[c(
  ...
  facet_wrap(~sex, 
             ncol = 1,
             scales = "free_y") 
  ggsave("output/RNAseqPlotsFinalReport/IREpval.png", width = 7, height = 5)
...
fry.cell[
 ...
 bind_rows() %>% 
  ...
  facet_wrap(~sex, 
             nrow = 1,
             scales = "free_x") +
  theme(axis.text.x = element_text(angle = -15, hjust = 0, vjust = 1, size = 18), 
        text = element_text(size = 18)) 
...
<!--   dplyr::filter(pathway %in% sigpaths_camera) %>%  -->

<!--   ggplot(aes(x = coef, y =  pathway, fill = FDR)) + -->
...
<!--   dplyr::filter(pathway %in% sigpaths_camera_males) %>%  -->
...
<!-- sigpathscam.contofinterest <- camera %>%  -->
...
<!-- dplyr::filter(pathway %in% sigpathscam.contofinterest) %>%  -->
...
<!-- # looking at some individual genes -->
...
<!--   dplyr::filter(gene_name == "tfr1a") %>% view -->
...
<!--   ggplot(aes(x = logFC, y = coef)) + -->

<!--   geom_col(aes(fill = DE)) +  -->
...
<!--   ggsave("output/RNAseqPlotsFinalReport/tfr1ab.png") -->
